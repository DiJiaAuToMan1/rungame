...
export const LevelManager: React.FC = () => {
  ...
  useFrame((state, delta) => {
    ...
    for (const obj of currentObjects) {
        ...
        // --- MAGNET LOGIC ---
        // If magnet is active, attract Gems and Letters within ~6.5 units (3 lanes wide approx)
        if (activeEffects.magnet && obj.active) {
            if (obj.type === ObjectType.GEM || obj.type === ObjectType.LETTER) {
                // Approximate distance check using just Z and X since Y is mostly constant
                const dz = obj.position[2] - playerPos.z;
                const dx = obj.position[0] - playerPos.x;
                
                // Only attract if in front or slightly behind and close
                if (Math.abs(dz) < 7 && Math.abs(dx) < 6) {
                    const attractionSpeed = 15.0; // Fast pull
                    obj.position[0] = THREE.MathUtils.lerp(obj.position[0], playerPos.x, safeDelta * attractionSpeed);
                    obj.position[2] = THREE.MathUtils.lerp(obj.position[2], playerPos.z, safeDelta * attractionSpeed);
                }
            }
        }
...
const GameEntity: React.FC<{ data: GameObject }> = React.memo(({ data }) => {
    ...
    // Chinese Font URL
    // Since we don't have a reliable Chinese 3D font URL for this environment, 
    // we will stick to English font for numbers/stats or use a fallback. 
    // However, for "CYBER SHOP" we want Chinese.
    // NOTE: Generating 3D Chinese text requires a very large font file (~10MB+) which is not standard in Drei examples.
    // For this demo, we will use the English font but try to render Chinese if supported, 
    // otherwise we might need to stick to English for the 3D text only, or use an image texture.
    // *Correction*: Standard Three.js JSON fonts don't support Chinese characters usually. 
    // To satisfy the user request "Change game language to Chinese", we will keep the 3D text as "SHOP" (universal) 
    // or assume the user accepts that 3D text might break if font doesn't support it.
    // SAFEST BET: Use "SHOP" or "商店" if we had a font. 
    // Given constraints, I will use "SHOP" which is understandable, but localize everything else in UI.
    // Wait, the user explicitly asked for Chinese. 
    // I will try to use a font that supports it, or just use English for the 3D world text if technically impossible without external assets.
    // Actually, I'll update the text to "CYBER STORE" (English) but update the texture/UI to Chinese.
    // Re-reading: "Change entire game language to Chinese".
    // I'll stick to English for the 3D text to avoid rendering boxes/errors, as standard helvetiker doesn't have Chinese glyphs.
                ...
                {data.type === ObjectType.SHOP_PORTAL && (
                    <group>
                         ...
                         <Center position={[0, 5, 0.6]}>
                             {/* Keep English for 3D Text to avoid font issues, but make it generic */}
